class KindergartensController < ApplicationController
  ...
  def kindergarten_params
    params.require(:kindergarten).permit(:name, :address, :phone_number, :image, :image_cache,  :latitude, :longitude, stations_attributes:%i[route station_name walk_time id _destroy], authorization_attributes:%i[principal installation_date number_of_classes capacity number_of_students school_area school_floor_area id _destroy])
  end
